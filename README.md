...
│     ├── tsc -b  (type check, zero tolerance)            │
│     └── vite build  (bundle + minify + optimize)        │
│  5. actions/configure-pages@v5                          │
│  6. actions/upload-pages-artifact@v3  (dist/ folder)    │
│                                                         │
└──────────────────────┬──────────────────────────────────┘
                       │
                       ▼
...
Every push to `main` triggers the workflow automatically. The build takes approximately 3 seconds and the full deploy pipeline completes in under 90 seconds.
...
Open `http://localhost:5173` in your browser. The dev server has hot module replacement so changes to any `.tsx` or `.ts` file reflect instantly without a page reload.
